...
def group_sentences(image_data, sentences):
    ...
    for i in range(n_boxes):
        ...
        if current_sentence:
            ...
            left_diff = image_data['left'][i] - \
              (image_data['left'][i - 1] + image_data['width'][i - 1])
            ...
            sentences[current_top]['height'] = \
                image_data['height'][i] + BLOCK_PADDING
            sentences[current_top]['left'] = \
                image_data['left'][i] - BLOCK_PADDING
            sentences[current_top]['width'] = \
                image_data['width'][i] + BLOCK_PADDING
...
def put_text_in_place(sentence, translated_text, image_array, top):
    (translated_w, translated_h), _ = cv2.getTextSize(
        translated_text,
        font,
        font_scale,
        thickness=1
    ...
    # There may be differences of width/height between the
...
def translate(sentences, image_array):
    for top, sentence in sentences.items():
        ...
        if language != LanguageAcronyms.SPANISH.value:
            sentence_text = translator.translate(
                sentence_text,
                LanguageAcronyms.SPANISH.value,
                language
            ).text
