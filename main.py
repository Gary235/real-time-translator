...
class App:
    def __init__(self, window, window_title):
        ...
        self.top_var = tkinter.StringVar(value='0')
        self.left_var = tkinter.StringVar(value='0')
        self.width_var = tkinter.StringVar(value='550')
        self.height_var = tkinter.StringVar(value='450')
        self.top_entry = ttk.Entry(
            self.frame,
            textvariable=self.top_var,
            validate="key",
            validatecommand=(self.window.register(validate_entry), "%S")
        )
    ...
    def update_capture(self):
        if not self.screen:
            self.canvas.config(
                width=int(self.width_var.get()) + 8,
                height=int(self.height_var.get()) + 8
            )
